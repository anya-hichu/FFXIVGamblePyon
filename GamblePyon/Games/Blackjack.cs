// GamblePyon, Version=1.0.0.7, Culture=neutral, PublicKeyToken=null
// GamblePyon.Modules.Blackjack
using System;
...
public class Blackjack
{
    ...
    private string FormatMessage(string message, Player player, Hand? hand = null)
    {
        ...
        return message.Replace("#dealer#", player.Alias).Replace("#player#", player.Alias).Replace("#firstname#", player.GetAlias(NameMode.First))
            ...
            .Replace("#value#", (hand == null) ? player.Blackjack.GetValues() : ((hand.AceLowValue == 0) ? hand.GetStrValue() : ((hand.AceHighValue < 21) ? $"{hand.AceLowValue}/{hand.AceHighValue}" : ((hand.AceHighValue == 21) ? $"{hand.AceHighValue}" : $"{hand.AceLowValue}"))))
    ...
    private void SendMessage(string message)
    {
        if (Enabled)
        {
            this.Send_Message(this, new MessageEventArgs(message, MessageType.Normal, MainTab.Blackjack));
        }
    }
    ...
    private void SendRoll()
    {
        if (Enabled)
        {
            this.Send_Message(this, new MessageEventArgs(Config.Blackjack.MaxRoll.ToString(), MessageType.BlackjackRoll, MainTab.Blackjack));
        }
    }
    ...
    public void OnChatMessage(string sender, string message)
    {
        ...
        try
        {
            ...
            {
                CurrentAction = Action.None;
                string cardValue2 = ((Config.RollCommand == "/dice") ? message.Replace("Random! (1-13) ", "") : Regex.Replace(message, ".*You roll a ([^\\(]+)\\(.*", "$1", RegexOptions.Singleline).Trim());
                Dealer!.Blackjack.Hands[0].Cards.Add(new Card(int.Parse(cardValue2), Config.Blackjack.ShowSuit));
                ...
                else if (Config.Blackjack.DealerStandMode != 0 && value >= GetStandValue())
                {
                    SendMessage(FormatMessage(Config.Blackjack.Messages["DealerDraw2Stand"], Dealer) ?? "");
                }
                else if (Config.Blackjack.DealerStandMode != 0)
                {
            ...
            {
                CurrentAction = Action.None;
                string cardValue3 = ((Config.RollCommand == "/dice") ? message.Replace("Random! (1-13) ", "") : Regex.Replace(message, ".*You roll a ([^\\(]+)\\(.*", "$1", RegexOptions.Singleline).Trim());
                Dealer!.Blackjack.Hands[0].Cards.Add(new Card(int.Parse(cardValue3), Config.Blackjack.ShowSuit));
                int value2 = Dealer.Blackjack.Hands[0].GetIntValue();
                if (value2 == 21)
                {
                    SendMessage(FormatMessage(Config.Blackjack.Messages["DealerHit21"], Dealer) ?? "");
                }
                else if (Config.Blackjack.DealerStandMode != 0 && value2 >= GetStandValue() && value2 < 21)
                {
                    SendMessage(FormatMessage(Config.Blackjack.Messages["DealerHitStand"], Dealer) ?? "");
                }
                else if (Config.Blackjack.DealerStandMode != 0 && value2 < GetStandValue())
                {
                    SendMessage(FormatMessage(Config.Blackjack.Messages["DealerHitUnderStand"], Dealer) ?? "");
                }
                else if (Config.Blackjack.DealerStandMode == DealerStandMode.None && value2 < 21)
                {
            ...
            else if (CurrentAction == Action.PlayerDraw2)
            {
                ...
                string cardValue4 = ((Config.RollCommand == "/dice") ? message.Replace("Random! (1-13) ", "") : Regex.Replace(message, ".*You roll a ([^\\(]+)\\(.*", "$1", RegexOptions.Singleline).Trim());
                CurrentHand.Cards.Add(new Card(int.Parse(cardValue4), Config.Blackjack.ShowSuit));
                if (CurrentHand.Cards.Count == 2)
                {
                    CurrentAction = Action.None;
                    if (CurrentHand.GetIntValue() == 21)
                    {
            ...
            {
                if (CurrentAction != Action.PlayerHit)
                {
                    return;
                }
                CurrentAction = Action.None;
                ...
                string cardValue5 = ((Config.RollCommand == "/dice") ? message.Replace("Random! (1-13) ", "") : Regex.Replace(message, ".*You roll a ([^\\(]+)\\(.*", "$1", RegexOptions.Singleline).Trim());
                CurrentHand.Cards.Add(new Card(int.Parse(cardValue5), Config.Blackjack.ShowSuit));
                int value3 = CurrentHand.GetIntValue();
                if (value3 == 21)
                {
    ...
    {
        return Config.Blackjack.DealerStandMode switch
        {
            DealerStandMode._16 => 16,
            DealerStandMode._17 => 17,
            _ => 0,
        };
    }
    ...
    {
        if (Config.Blackjack.DealerStandMode != 0 || Dealer!.Blackjack.Hands[0].GetIntValue() >= 21)
        {
            if (Config.Blackjack.DealerStandMode != 0)
            {
                return Dealer!.Blackjack.Hands[0].GetIntValue() < GetStandValue();
            }
            return false;
        }
        return true;
    ...
    {
        if (Dealer!.Blackjack.Hands[0].GetIntValue() <= 21)
        {
            if (Config.Blackjack.DealerStandMode != 0)
            {
                if (Config.Blackjack.DealerStandMode != 0)
    ...
    private void DrawGame()
    {
        ...
        {
            this.Close_Window(this, new EventArgs());
        }
    }
    ...
    {
        ImGui.Columns(2);
        ImGui.SetColumnWidth(0, 90f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.SetColumnWidth(1, 230f + 5f * ImGuiHelpers.GlobalScale);
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("This is you! ..Or at least it should be.\nI dunno what would happen if it's not.");
        }
        ImGui.Columns(9);
        ImGui.SetColumnWidth(0, 55f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.SetColumnWidth(1, 90f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.SetColumnWidth(2, 80f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.SetColumnWidth(3, 65f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.SetColumnWidth(4, 70f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.SetColumnWidth(5, 75f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.SetColumnWidth(6, 45f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.SetColumnWidth(7, 65f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.SetColumnWidth(8, 80f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.Separator();
        ImGui.Text("");
        ImGui.NextColumn();
        ImGui.Text("Alias");
        ...
        if (ImGui.Button("Rules"))
        {
            ...
            SendMessage(FormatMessage(Config.Blackjack.Messages_Rules["DealerStand_" + ((Config.Blackjack.DealerStandMode != 0) ? "True" : "False")], Dealer) ?? "");
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Output the rules!");
        }
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("The total bet pool for this round.");
        }
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        if (Enabled && !string.IsNullOrWhiteSpace(Dealer.Alias) && (CurrentEvent == Event.PlaceBets || (CurrentEvent == Event.BetsPlaced && Players.Find((Player x) => x.TotalBet > 0) != null)))
        {
            string label = ((CurrentEvent != Event.BetsPlaced) ? "B" : "F");
            string btnMsg = ((CurrentEvent != Event.BetsPlaced) ? Config.Blackjack.Messages["PlaceBets"] : Config.Blackjack.Messages["BetsPlaced"]);
            string hoverMsg2 = ((CurrentEvent != Event.BetsPlaced) ? "Request players to place bets." : "Announce all bets are placed.");
            if (ImGui.Button(label))
            {
                ...
                else if (CurrentEvent == Event.BetsPlaced)
                {
                    ...
                    {
                        player2.BetPerHand = player2.TotalBet;
                    }
        ...
        {
            if (Dealer.Blackjack.Hands[0].Cards.Count == 0 && Players.Find((Player x) => x.Alias != "" && x.TotalBet != 0 && x.Blackjack.Hands[0].Cards.Count != 2) == null)
            {
                if (ImGui.Button("1"))
                {
                ...
                if (ImGui.IsItemHovered())
                {
                    ImGui.SetTooltip("After player initial 2 cards, draw dealer 1st card.");
                }
            }
            else if (Dealer.Blackjack.Hands[0].Cards.Count > 0)
            {
                string label2 = ((Dealer.Blackjack.Hands[0].Cards.Count == 1) ? "2" : "H");
        ...
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        string cards = Dealer.Blackjack.Hands[0].GetCards(forceOnlyValue: true);
        ...
        Player playerValue = Players.Find((Player x) => !string.IsNullOrWhiteSpace(x.Alias) && x.Blackjack.Hands.Find((Hand x) => x.Cards.Count > 0 && x.GetIntValue() > dealerValue && x.GetIntValue() <= 21) != null)!;
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("The total profit/loss for the dealer.\n" + areYouOk);
        }
        ImGui.NextColumn();
    }
        ...
        foreach (Player player in Players!)
        {
            foreach (Hand hand in player.Blackjack.Hands)
            {
                ...
                if (Players.Count > 1 && handIndex == 0)
                {
                    ...
                    {
                        ImGui.Dummy(new Vector2(20f, 10f));
                    }
                    ImGui.SameLine();
                ...
                if (Enabled && !string.IsNullOrWhiteSpace(player.Alias) && Config.Blackjack.AllowSplit && handIndex == 0 && hand.Cards.Count == 2 && Dealer!.Blackjack.Hands[0].Cards.Count == 1 && player.Blackjack.Hands.Count == 1 && hand.Cards[0].TextNoSuit == hand.Cards[1].TextNoSuit && hand.GetIntValue() < 21)
                {
                    ...
                    {
                        List<Hand> hands = player.Blackjack.Hands;
                        Hand hand2 = new Hand();
                        int num = 1;
                        List<Card> list = new List<Card>(num);
                        CollectionsMarshal.SetCount(list, num);
                        Span<Card> span = CollectionsMarshal.AsSpan(list);
                        int num2 = 0;
                        span[num2] = hand.Cards[1];
                        num2++;
                        hand2.Cards = list;
                        hands.Add(hand2);
                ...
                ImGui.NextColumn();
                ImGui.SetNextItemWidth(-1f);
                string cards = hand.GetCards(forceOnlyValue: true);
                ...
                ImGui.NextColumn();
                ImGui.SetNextItemWidth(-1f);
                if (Enabled && !string.IsNullOrWhiteSpace(player.Alias) && handIndex == 0 && player.TotalBet > 0 && Dealer!.Blackjack.Hands[0].Cards.Count >= 2 && player.Blackjack.Hands.Find((Hand x) => x.Cards.Count < 2) == null)
                {
                    int dealerValue = Dealer.Blackjack.Hands[0].GetIntValue();
                    if (Config.Blackjack.DealerStandMode == DealerStandMode.None || dealerValue >= GetStandValue())
                    {
                        int h1Result = 0;
                        int h2Result = 0;
                        double hWinnings = 0.0;
                        double hLoss = 0.0;
                        foreach (var h in player.Blackjack.Hands)
                        {
                            ...
                            if ((handValue > dealerValue || dealerValue > 21) && handValue <= 21)
                            {
                                ...
                                hWinnings += (double)((handValue == 21 && (!Config.Blackjack.NaturalBlackjack || h.Cards.Count == 2) && player.Blackjack.Hands.Count == 1) ? ((float)player.TotalBet * Config.Blackjack.BlackjackWinMultiplier) : ((float)(h.Doubled ? (player.BetPerHand * 2) : player.BetPerHand) * Config.Blackjack.NormalWinMultiplier));
                            }
                            else if (handValue < dealerValue || handValue > 21)
                            {
                                hLoss += (double)(h.Doubled ? (player.BetPerHand * 2) : player.BetPerHand);
                            }
    ...
    private void DrawConfigGameSetup()
    {
        ...
        ImGui.Columns(3);
        ImGui.SetColumnWidth(0, 200f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.SetColumnWidth(1, 200f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.SetColumnWidth(2, 200f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.Separator();
        ImGui.Text("Min Bet");
        ImGui.NextColumn();
        ImGui.Text("Max Bet");
        ...
        ImGui.Separator();
        ImGui.SetNextItemWidth(-1f);
        ImGuiEx.InputInt("###minBet", Config.Blackjack, "MinBet");
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ImGuiEx.InputInt("###maxBet", Config.Blackjack, "MaxBet");
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Dealer plays without stand limitation.");
        }
        ImGui.SameLine();
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Dealer must stand when value is 16 or over.");
        }
        ImGui.SameLine();
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Dealer must stand when value is 17 or over.");
        }
        ImGui.NextColumn();
        ImGui.Separator();
        ImGui.SetNextItemWidth(-1f);
        ImGuiEx.Checkbox("Allow Split", Config.Blackjack, "AllowSplit");
        if (ImGui.IsItemHovered())
        {
        ...
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ImGuiEx.Checkbox("Double Must Hit", Config.Blackjack, "DoubleMustHit");
        if (ImGui.IsItemHovered())
        {
        ...
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ImGuiEx.Checkbox("Allow Bet on Push", Config.Blackjack, "PushAllowBet");
        if (ImGui.IsItemHovered())
        {
        ...
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ImGuiEx.Checkbox("Blackjack Win Multiplier", Config.Blackjack, "NaturalBlackjack");
        if (ImGui.IsItemHovered())
        {
        ...
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ImGuiEx.Checkbox("Allow Double on Push", Config.Blackjack, "PushAllowDouble");
        if (ImGui.IsItemHovered())
        {
        ...
        ImGui.Separator();
        ImGui.SetNextItemWidth(-1f);
        ImGuiEx.InputFloat("###normalWin", Config.Blackjack, "NormalWinMultiplier");
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Win multiplier for when a player wins with a hand less than 21" + (Config.Blackjack.NaturalBlackjack ? ", or 21 with more than 2 cards." : "."));
        }
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ImGuiEx.InputFloat("###blackjackWin", Config.Blackjack, "BlackjackWinMultiplier");
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Win multiplier for when a player wins with a " + (Config.Blackjack.NaturalBlackjack ? "natural blackjack (21 with 2 cards)." : "hand valued 21."));
        }
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ImGuiEx.Checkbox("Show Suits###showSuit", Config.Blackjack, "ShowSuit");
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Some boring people don't like these symbols ;w;\nDisabling this will make the boring people happy..\n..but Pyon will be sad.");
        }
        ImGui.NextColumn();
    }
    ...
    private void DrawConfigRulesMessages()
    {
        ...
        ImGui.Columns(2);
        ImGui.SetColumnWidth(0, 340f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.SetColumnWidth(1, 340f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.Separator();
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Rules Title Header");
        }
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ImGuiHelpers.ScaledDummy(5f);
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Normal Win Multiplier");
        }
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ImGuiHelpers.ScaledDummy(5f);
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Blackjack Win Multiplier\nOnly offered for Natural Blackjack");
        }
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Dealer Stands On: 16/17");
        }
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Dealer Stands On: None");
        }
        ImGui.NextColumn();
        ImGui.Separator();
        ImGui.SetNextItemWidth(-1f);
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Double Must Hit: True");
        }
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Double Must Hit: False");
        }
        ImGui.NextColumn();
        ImGui.Separator();
        ImGui.SetNextItemWidth(-1f);
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Allow Split: True");
        }
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Allow Split: False");
        }
        ImGui.NextColumn();
        ImGui.Separator();
        ImGui.SetNextItemWidth(-1f);
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Allow Bet on Push: True");
        }
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Allow Bet on Push: False");
        }
        ImGui.NextColumn();
        ImGui.Separator();
        ImGui.SetNextItemWidth(-1f);
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Allow Double on Push: True");
        }
        ImGui.NextColumn();
        ImGui.SetNextItemWidth(-1f);
        ...
        if (ImGui.IsItemHovered())
        {
            ImGui.SetTooltip("Allow Double on Push: False");
        }
        ImGui.NextColumn();
        ImGui.Separator();
        ImGui.PopID();
    ...
    private void DrawConfigMessages()
    {
        ...
        ImGui.Columns(2);
        ImGui.SetColumnWidth(0, 180f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.SetColumnWidth(1, 500f + 5f * ImGuiHelpers.GlobalScale);
        ImGui.Separator();
        ...
        foreach (KeyValuePair<string, string> message in Config.Blackjack.Messages)
        {
            ...
            if (message.Key == "PlayerStandHit")
            {
                if (ImGui.IsItemHovered())
                {
                    ImGui.SetTooltip("This might look the same as PlayerHitUnder21\n..but it's not the same, trust me.");
                }
            }
            else if (message.Key == "Loss" && ImGui.IsItemHovered())
            {
                ImGui.SetTooltip("Should probably not let people know when they lose.\n..it might make them realize they're losing, you know?");
            }
            ImGui.NextColumn();
            ImGui.Separator();
        }
