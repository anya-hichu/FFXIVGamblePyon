...
namespace GamblePyon {
    public unsafe class PartyManager : IDisposable {
        ...
        public PartyManager() {
            UpdateParty updateParty = new(GamblePyon.SigScanner);
        ...
        public void UpdatePlayers(ref List<Player> players, Player dealer, NameMode nameMode) {
            List<Player> partyMembers = new List<Player>();

            for(int i = 0; i < 200; i += 2) {
                var obj = GamblePyon.Objects[i];
